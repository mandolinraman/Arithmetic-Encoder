...
function compress(endec::Endec, message::String, len::Int = typemax(Int))
	...
	while m < length(message) && n < len
		...
		high = mod(low + divmod_abc(span, cmf + prob, sum_freq)[1] + (index + 1) * endec.delta - 1, full)
		low = mod(low + divmod_abc(span, cmf, sum_freq)[1] + index * endec.delta, full)
...
# test 2
println("\nTest 2\n======")
message = read("arithmetic_coding_alt.py", String)
test(message)
