...
class ArithmeticCoder:
    ...
    def encode(self, message):
        ...
        for j, sym in enumerate(message):
            ...
            while True:
                # convert inverted state to noninverted if possible
                if low >= half and high < half:
                    low -= half
                    high += half
                    num_inversions -= 1
                    if istate == -1:
                    ...
                    # print(f" Flipped: [{low/full}, {(high+1)/full}) | istate = {istate}")
                    # continue
                elif high < low:
                    # we're still inverted
                    break

                # noninverted istate: check if we can double interval
                # assert low <= high
                if high < half:
                    if istate == 1 and low < oldlow:
                        # if we came from istate 1, there's extra work:
                        num_inversions -= 1
                        bit_plus_inversions(1)
                        bit_plus_inversions(1)
                    else:
                    ...
                    if low < quarter:
                        istate = -1
                    elif high >= three_quarters:
                        istate = 1
                    else:
                        istate = 0
                    # may cause inversion
                    low = (low - quarter) % half
                    high = (high - quarter) % half
                    num_inversions += 1
        ...
        for j in range(mlen):
            ...
            while True:
                ...
                if low >= half and high < half:
                    ...
                    low -= half
                    high += half
                    # print(f" Flipped: [{low/full}, {(high+1)/full}) | codeval = {value/full}")
                elif high < low:
                    # we're still inverted
                    break

                # noninverted state: check if we can double interval
                # assert low <= high
                if high < half:
                    pass
                elif low >= half:
